...
class EditorPanel(tk.Toplevel):
    ...
    def _build(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="GURT Editor Panel", bg="#0d0d3a", fg="white",
        ...
        self._brush_btn = tk.Radiobutton(
            mode_frame, text="🖌️  Unfold Brush",
        ...
        r2.pack(fill="x", **pad)
        self._lbl(r2, "Fold centre (m/s):").pack(side="left")
...
class GURTApp(tk.Tk):
    def __init__(self, folder=None, file=None):
        ...
        # ── Editor / brush state ──────────────────────────────────────────────
        self._editor_mode       = tk.StringVar(value="zoom")  # "zoom" | "unfold_brush" | "deglitch_brush"
    ...
    def _on_press(self, event):
        if event.button == 1 and event.inaxes:
            ...
            if mode in ("unfold_brush", "deglitch_brush"):
    ...
    def _on_release(self, event):
        if event.button == 1 and self._editor_mode.get() in ("unfold_brush", "deglitch_brush"):
            self._brush_active = False
            # Full quality redraw once the stroke is complete
            self.refresh_plot()
            return
    ...
    def _apply_unfold_brush(self, ax, x_km, y_km):
        ...
        # Snapshot current state for undo before modifying anything
